...
function assertParamsAreAvailable(tokens, params) {
  ...
  for (const token of tokens.filter(t => t.type === 1)) {
    for (const condition of token.rule.conditions) {
      ...
      if (evaluator.isQueryParameter(head)) {
...
module.exports.normalizeCondtion = condition => {
  ...
  if (evaluator.isCustomAttributeKey(condition.key)) {
    key = `attributes.${condition.key}`
  } else if (evaluator.isQueryParameter(condition.key)) {
...
module.exports.isCustomAttributeKey = evaluator.isCustomAttributeKey

module.exports.isQueryParameter = evaluator.isQueryParameter
